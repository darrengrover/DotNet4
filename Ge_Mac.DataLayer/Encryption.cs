...
namespace Ge_Mac.DataLayer
{
    ...
    public class Encryption
    {
        public static void Test()
        {
            ...
            // Load an XML file into an XmlDocument object.
            ...
            {
                Trace.WriteLine("RAIL: " + ex.Message);
            }
            ...
            try
            {
                ...
                Debug.WriteLine("RAIL: Encrypted XML:");
                Debug.WriteLine("");
            ...
            string EncryptionElementID, RSA Alg, string KeyName)
        {
            ...
            try
            {
                ...
                KeyInfoName kin = new KeyInfoName(KeyName);
    ...
    /// <summary>This class uses a symmetric key algorithm (Rijndael/AES) to encrypt and
    ...
    /// vector, and other parameters. This class is thread-safe.</summary>
    /// <remarks>Be careful when performing encryption and decryption. There is a bug
    ...
    /// instance when a cryptographic exception occurs.</remarks>
    public class RijndaelEnhanced
    {
        ...
        #region Constructors
        /// <summary>Use this constructor if you are planning to perform encryption/
        ...
        /// (ECB) mode, SHA-1 hashing algorithm, and 4-to-8 byte long salt.</summary>
        ...
        /// passphrase is an ASCII string. Passphrase value must be kept in secret.</param>
        /// <remarks>This constructor is not recommended because it does not use
        /// initialization vector and uses the ECB cipher mode, which is less
        /// secure than the CBC mode.</remarks>
        public RijndaelEnhanced(string passPhrase)
            : this(passPhrase, null)
        {
        }

        /// <summary>Use this constructor if you are planning to perform encryption/decryption
        /// with 256-bit key, derived using 1 password iteration,
        /// hashing without salt, cipher block chaining (CBC) mode, SHA-1
        /// hashing algorithm, and 4-to-8 byte long salt.</summary>
        /// <param name="passPhrase">Passphrase from which a pseudo-random password will be derived.
        /// The derived password will be used to generate the encryption key.
        /// Passphrase can be any string. In this example we assume that the
        /// passphrase is an ASCII string. Passphrase value must be kept in
        /// secret.</param>
        /// <param name="initVector">Initialization vector (IV). This value is required to encrypt the
        ...
        /// decryption with 256-bit key, derived using 1 password iteration,
        /// hashing without salt, cipher block chaining (CBC) mode, SHA-1 
        /// hashing algorithm, and 0-to-8 byte long salt.</summary>
        /// <param name="passPhrase">Passphrase from which a pseudo-random password will be derived.
        /// The derived password will be used to generate the encryption key
        /// Passphrase can be any string. In this example we assume that the
        /// passphrase is an ASCII string. Passphrase value must be kept in
        ...
        /// exactly 16 ASCII characters long. IV value does not have to be kept
        /// in secret.</param>
        /// <param name="minSaltLen">Min size (in bytes) of randomly generated salt which will be added at
        ...
        /// specify the size of randomly generated salt.</summary>
        /// <param name="passPhrase">Passphrase from which a pseudo-random password will be derived.
        /// The derived password will be used to generate the encryption key.
        /// Passphrase can be any string. In this example we assume that the
        /// passphrase is an ASCII string. Passphrase value must be kept in
        ...
        /// SHA-1 hashing algorithm.</summary>
        /// <param name="passPhrase">Passphrase from which a pseudo-random password will be derived.
        /// The derived password will be used to generate the encryption key.
        /// Passphrase can be any string. In this example we assume that the
        /// passphrase is an ASCII string. Passphrase value must be kept in
        ...
        /// without salt, and cipher block chaining (CBC) mode.</summary>
        /// <param name="passPhrase">Passphrase from which a pseudo-random password will be derived.
        /// The derived password will be used to generate the encryption key.
        /// Passphrase can be any string. In this example we assume that the
        /// passphrase is an ASCII string. Passphrase value must be kept in secret.</param>
        /// <param name="initVector">Initialization vector (IV). This value is required to encrypt the
        /// first block of plaintext data. For RijndaelManaged class IV must be
        /// exactly 16 ASCII characters long. IV value does not have to be kept in secret.</param>
        /// <param name="minSaltLen">Min size (in bytes) of randomly generated salt which will be added at
        /// the beginning of plain text before encryption is performed. When this
        /// value is less than 4, the default min value will be used (currently 4 bytes).</param>
        /// <param name="maxSaltLen">Max size (in bytes) of randomly generated salt which will be added at
        /// the beginning of plain text before encryption is performed. When this
        /// value is negative or greater than 255, the default max value will be
        /// used (currently 8 bytes). If max value is 0 (zero) or if it is smaller
        ...
        public RijndaelEnhanced(string passPhrase, string initVector, int minSaltLen, int maxSaltLen,
             int keySize, string hashAlgorithm)
        ...
        /// cipher block chaining (CBC) mode.</summary>
        /// <param name="passPhrase">Passphrase from which a pseudo-random password will be derived.
        /// The derived password will be used to generate the encryption key.
        /// Passphrase can be any string. In this example we assume that the
        /// passphrase is an ASCII string. Passphrase value must be kept in secret.</param>
        ...
        /// not the same as the salt we will use during encryption. This parameter can be any string.</param>
        public RijndaelEnhanced(string passPhrase, string initVector, int minSaltLen, int maxSaltLen,
            int keySize, string hashAlgorithm, string saltValue)
            : this(passPhrase, initVector, minSaltLen, maxSaltLen, keySize, hashAlgorithm, saltValue, 1)
        {
        }

        /// <summary>Use this constructor if you are planning to perform encryption/
        /// decryption with the key derived from the explicitly specified parameters.</summary>
        /// <param name="passPhrase">Passphrase from which a pseudo-random password will be derived.
        /// The derived password will be used to generate the encryption key
        /// Passphrase can be any string. In this example we assume that the
        /// passphrase is an ASCII string. Passphrase value must be kept in secret.</param>
        ...
        public RijndaelEnhanced(string passPhrase, string initVector, int minSaltLen, int maxSaltLen,
            int keySize, string hashAlgorithm, string saltValue, int passwordIterations)
        {
        ...
        /// <summary>Encrypts a string value generating a base64-encoded string.</summary>
        ...
        /// <summary>Encrypts a byte array generating a base64-encoded string.</summary>
        ...
        /// <summary>Encrypts a string value generating a byte array of cipher text.</summary>
        /// <param name="plainText">Plain text string to be encrypted.</param>
        ...
        /// <summary>Encrypts a byte array generating a byte array of cipher text.</summary>
        /// <param name="plainTextBytes">Plain text bytes to be encrypted.</param>
        /// <returns>Cipher text formatted as a byte array.</returns>
        ...
        /// <summary>Decrypts a base64-encoded cipher text value generating a string result.</summary>
        ...
        /// <summary>Decrypts a byte array containing cipher text value and generates a string result.</summary>
        ...
        /// <summary>Decrypts a base64-encoded cipher text value and generates a byte array of plain text data.</summary>
        /// <param name="cipherText">Base64-encoded cipher text string to be decrypted.</param>
        ...
        /// <summary>Adds an array of randomly generated bytes at the beginning of the
        /// array holding original plain text value.</summary>
        /// <param name="plainTextBytes">Byte array containing original plain text value.</param>
        /// <returns>Either original array of plain text bytes (if salt is not used) or a
        /// modified array containing a randomly generated salt added at the 
        /// beginning of the plain text bytes.</returns>
        private byte[] AddSalt(byte[] plainTextBytes)
        {
            // The max salt value of 0 (zero) indicates that we should not use salt.
            // Also do not use salt if the max salt value is smaller than the min value.
            if (maxSaltLen == 0 || maxSaltLen < minSaltLen)
            {
                return plainTextBytes;
            }
        ...
        /// <summary>Generates an array holding cryptographically strong bytes.</summary>
        /// <returns>Array of randomly generated bytes.</returns>
        /// <remarks>Salt size will be defined at random or exactly as specified by the
        ...
        /// split into four two-bit pieces.</remarks>
        ...
        /// <summary>Generates random integer.</summary>
        ...
        /// <remarks>This methods overcomes the limitations of .NET Framework's Random
        /// class, which - when initialized multiple times within a very short
        /// period of time - can generate the same "random" number.</remarks>
    ...
    /// <summary>Illustrates the use of RijndaelEnhanced class to encrypt and decrypt data
    /// using a random salt value.</summary>
    public class RijndaelEnhancedTest
    {
        ...
        static void Main()
        {
            ...
            // Before encrypting data, we will append plain text to a random salt value,
            // which will be between 4 and 8 bytes long (implicitly used defaults).
            RijndaelEnhanced rijndaelKey = new RijndaelEnhanced(passPhrase, initVector);

            Debug.WriteLine(String.Format("RAIL: Plaintext   : {0}\n", plainText));
